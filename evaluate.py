...
def semeval_scorer(predict_label, true_label, class_num=6):
    import math
    ...
    for i in range(1, class_num):
        if f1[i] > 0.0:  # classes that not in the predict label are not considered
            ...
            total_f1 += f1[i]
    try:
        micro_f1 = total_f1 / actual_class
    except:
        micro_f1 = 0.0
    return micro_f1
...
class Eval(object):
    ...
    def evaluate(self, model, criterion, data_loader):
        ...
        #print("Micro: " + str(precision_recall_fscore_support(true_label, predict_label, average='micro')))
        #print("Macro: " + str(precision_recall_fscore_support(true_label, predict_label, average='macro')))
        #print("Weighted: " + str(precision_recall_fscore_support(true_label, predict_label, average='weighted')))
        #tmpArr = precision_recall_fscore_support(true_label, predict_label)
